...
(function($, undefined) {
	...
	jPlayerPlaylist.prototype = {
		...
		_createListItem: function(media) {
			...
			var audio = new Audio(media.mp3);
			var audioDuration = 0;
			var random = Math.floor(Date.now() * Math.random()) + 1;
			audio.onloadedmetadata = function() {
				audioDuration = new Date(audio.duration * 1000).toISOString().substr(14, 5);
				if(document.getElementById(`hbag-${random}`)){
					document.getElementById(`hbag-${random}`).innerText = audioDuration;
				}
			};

			var self = this;
			// Wrap the <li> contents in a <div>
			var listItem = `<li><div class='hbag-playslist-flex'><div class="hbag-time-duration"><span id="hbag-${random}">00:00</span></div>`;
