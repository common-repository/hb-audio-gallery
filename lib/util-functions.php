<?php
function hbag_arrToTable($arr) {
    if($arr){
        $str = "<br/><table border ='1'><tr><th>key</th><th>value</th></tr>";
        foreach ($arr  as $key=>$value){
            $str.="<tr><td>".$key."</td>";
            if(is_array($value)){
                $str .="<td>".hbag_arrToTable($value)."</td>";
            }else{
                $str.="<td>".$value."</td>";
            }
            $str.="</tr>";
...
function hbag_console_log( $message ) {
...
function hbag_get_filesize( $url ) {
    /*if( substr( $url, 0, 4 ) == 'http' ) {
        $x = array_change_key_case( get_headers( $url, 1 ), CASE_LOWER );
        if( strpos( $x[0], '404 Not Found' ) != false ) {
            $path = hbag_convert_urltopath( $url );
            $x = filesize( $path );
...
function hbag_copyfile( $oldfile, $newfile ) {
...
function hbag_convert_urltopath( $url_addr ) {
    $path_addr = str_replace( get_site_url() . '/', HBAG_WINABSPATH, $url_addr );
    return $path_addr;
}

function hbag_convert_pathtourl($url_addr)
{
    // $path_addr = str_replace( HBAG_WINABSPATH, get_site_url() . '/', $url_addr );
    $path_addr = str_replace('\\\\', '/', $url_addr);
    
    return str_replace(HBAG_WINABSPATH, get_site_url(). '/', $path_addr);
}
function hbag_remove_dir( $path ) {
    if( is_file($path) ) {
        unlink($path);
    } elseif ( is_dir($path) ) {
        $dir = opendir($path);
...
function hbag_WriteNewHtaccess($htaccess_new_content){
